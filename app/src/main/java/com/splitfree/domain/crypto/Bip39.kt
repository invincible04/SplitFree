...
/**
 * BIP-39 mnemonic implementation — from scratch, no SDK.
 * Converts 32-byte entropy (private key) ↔ 24-word mnemonic.
 * Uses the standard English wordlist (2048 words).
 */
object Bip39 {
    ...
    fun toEntropy(words: List<String>): ByteArray {
        ...
        val bits = words.joinToString("") { word ->
            val index = WORD_INDEX[word.lowercase()]
    ...
    private fun toBitString(bytes: ByteArray): String =
        bytes.joinToString("") { (it.toInt() and 0xFF).toString(2).padStart(8, '0') }
