...
class EventProcessor @Inject constructor(
    ...
    data class ProcessResult(val stored: Boolean, val groupName: String? = null,
                             val eventType: String? = null, val decrypted: String? = null,
    ...
    suspend fun process(
        ...
        val timestampValid = if (lenientTimestamp) EventValidator.isTimestampValidLenient(inner.createdAt)
            else EventValidator.isTimestampValid(inner.createdAt)
        if (!timestampValid) {
    ...
    private suspend fun handleSideEffects(
        eventType: String, decrypted: String?, authorHex: String,
        groupId: String, createdAt: Long, nonCancellable: Boolean
    ) {
        ...
        when (eventType) {
            ...
            catch (e: Exception) { Log.e(TAG, "Failed to process group_meta for $groupId: ${e.message}", e) }
