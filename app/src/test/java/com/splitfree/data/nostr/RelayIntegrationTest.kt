...
class RelayIntegrationTest {
    ...
    @Test
    fun `relay connects to live server`() =
        runBlocking {
    ...
    @Test
    fun `relay publish and receive OK response`() =
        runBlocking {
            relay = Relay(relayUrl, scope)
            relay.connect()
    ...
    @Test
    fun `relay subscribe receives EOSE`() =
        runBlocking {
            relay = Relay(relayUrl, scope)
            relay.connect()
            withTimeout(10_000) { relay.state.first { it == Relay.State.CONNECTED } }
    ...
    @Test
    fun `relay subscribe receives events before EOSE`() =
        runBlocking {
            relay = Relay(relayUrl, scope)
            relay.connect()
            withTimeout(10_000) { relay.state.first { it == Relay.State.CONNECTED } }
    ...
    @Test
    fun `relay disconnect transitions to DISCONNECTED`() =
        runBlocking {
            relay = Relay(relayUrl, scope)
            relay.connect()
            withTimeout(10_000) { relay.state.first { it == Relay.State.CONNECTED } }
    ...
    @Test
    fun `NostrClient full round-trip with live relay`() =
        runBlocking {
    ...
    @Test
    fun `NostrClient rejects non-wss relay URLs`() =
        runBlocking {
            val client = NostrClient()
            try {
    ...
    @Test
    fun `relay handles invalid URL gracefully`() =
        runBlocking {
