...
class ExpenseRepositoryTest {
    ...
    fun `addExpense encrypts and stores event`() = runTest {
        val expense = Expense("u1", 100, "INR", "test", "alice", SplitType.EQUAL,
    ...
    fun `addExpense throws when no group key`() = runTest {
        ...
        repo().addExpense(
            Expense("u1", 100, "INR", "t", "a", SplitType.EQUAL, listOf(SplitEntry("a", 100)), 1), "g1"
        )
    }
    ...
    fun `correctExpense stores correction event`() = runTest {
        ...
        val corrected = Expense("u1c", 200, "INR", "corrected", "alice", SplitType.EQUAL,
            listOf(SplitEntry("alice", 100), SplitEntry("bob", 100)), 2)
    ...
    @Test
    fun `addExpense with gift wrap wraps for each member`() = runTest {
