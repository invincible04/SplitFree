...
class MigrateGroupUseCaseTest {
    ...
    fun setup() {
        ...
        useCase = MigrateGroupUseCase(groupRepo, encryption, identity, signer, eventDao, eventPublisher)
    }
    ...
    fun `handleMigration ignores if new group already exists`() = runBlocking {
        every { identity.getPublicKeyHex() } returns memberPubkey
        coEvery { groupRepo.getById("new-id") } returns
            Group("new-id", "G", "", myPubkey, 1000, listOf(myPubkey), listOf("wss://r"))
        val migration =
            GroupMigration(
                newGroupId = "new-id",
