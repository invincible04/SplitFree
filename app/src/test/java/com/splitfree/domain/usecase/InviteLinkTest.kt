...
class InviteLinkTest {
    ...
    // Known relays — must match JoinGroupUseCase.KNOWN_RELAYS exactly
    ...
    private fun decodeCompactLink(fragment: String): Map<String, String> {
        ...
        return mapOf("groupId" to groupId, "groupKey" to groupKey, "relays" to relays.joinToString(","), "name" to name, "exp" to exp.toString())
    }
    ...
    fun `v2 link with all 5 known relays uses bitmap only`() {
        val link = createCompactLink(
            "56a0833d-a77c-418f-ac2d-64150216af31", "key1", KNOWN_RELAYS, "Test"
        )
        val fragment = link.substringAfter("d=")
    ...
    fun `v2 link with unicode group name`() {
        val link = createCompactLink("56a0833d-a77c-418f-ac2d-64150216af31", "key1", listOf("wss://nos.lol"), "गोवा ट्रिप 🏖️")
        val decoded = decodeCompactLink(link.substringAfter("d="))
    ...
    fun `v1 legacy link round-trip preserves fields`() {
        val g = Base64.getUrlEncoder().withoutPadding().encodeToString("56a0833d-a77c-418f-ac2d-64150216af31".toByteArray())
