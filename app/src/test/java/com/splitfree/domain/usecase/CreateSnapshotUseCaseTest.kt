...
class CreateSnapshotUseCaseTest {
    ...
    fun `skips when below threshold`() = runBlocking {
        ...
        val recentSnapshot = EventEntity(
            eventId = "snap1", groupId = groupId, pubkey = "pub",
            createdAt = System.currentTimeMillis() / 1000 - 86400, // 1 day ago
            kind = 30078, contentEncrypted = "enc",
            contentDecrypted = """{"id":"s1","as_of_event_count":40,"as_of_timestamp":1,"balances":[],"event_hashes":[]}""",
            eventType = "snapshot", sig = "sig", receivedAt = 1
        )
