...
class RealRelayExpenseFlowTest {
    ...
    private fun generateValidPrivateKey(): ByteArray {
        ...
        do { random.nextBytes(key) } while (!fr.acinq.secp256k1.Secp256k1.secKeyVerify(key))
        return key
    }
    ...
    fun `full expense flow - create group, add expenses, verify cross-phone sync`() = runBlocking {
        ...
        val metaEvent = phone1Signer.createSignedEvent(groupId, "group_meta",
        ...
        val expense1 = Expense(
            ...
            description = "Dinner at restaurant", paidBy = phone1PubKey,
            splitType = SplitType.EQUAL,
            splitAmong = listOf(SplitEntry(phone1PubKey, 75000), SplitEntry(phone2PubKey, 75000)),
            timestamp = now, category = "food",
        )
        val expense1Event = phone1Signer.createSignedEvent(
            groupId = groupId, eventType = "expense",
