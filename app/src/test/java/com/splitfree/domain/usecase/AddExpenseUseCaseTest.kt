...
class AddExpenseUseCaseTest {
    ...
    @Test
    fun `valid expense calls repository`() =
        runTest {
            val repo = repo()
            val uc = useCase(repo)
            uc(
    ...
    @Test
    fun `expense has correct fields passed to repo`() =
        runTest {
            val repo = repo()
    ...
    @Test(expected = IllegalArgumentException::class)
    fun `amount exceeding max throws`() =
        runTest {
            useCase()(
                "g1",
                10_000_000_000_01L,
    ...
    @Test(expected = IllegalArgumentException::class)
    fun `shares not summing to amount throws`() =
        runTest {
            useCase()(
                "g1",
                100,
    ...
    @Test(expected = IllegalArgumentException::class)
    fun `numeric currency throws`() =
        runTest {
            useCase()("g1", 100, "123", "t", "a", SplitType.EQUAL, listOf(SplitEntry("a", 100)))
        }

    @Test
    fun `currency is normalized to uppercase trimmed`() =
        runTest {
            val repo = repo()
            val slot = slot<Expense>()
            coEvery { repo.addExpense(capture(slot), any()) } just Runs
    ...
    @Test
    fun `no repo call on validation failure`() =
        runTest {
            val repo = repo()
            try {
