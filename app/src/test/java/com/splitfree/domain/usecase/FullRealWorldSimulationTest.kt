...
class FullRealWorldSimulationTest {
    ...
    private fun makeExpense(
        id: String, amount: Long, currency: String, desc: String,
        paidBy: String, splitType: SplitType, splits: List<SplitEntry>,
        ts: Long, category: String,
    ...
    private fun signExpense(signer: EventSigner, expense: Expense): NostrEvent {
        ...
        return signer.createSignedEvent(groupId, "expense",
            encryption.encrypt(plain, groupKey), expense.id)
    }
    ...
    private fun signDeletion(signer: EventSigner, expenseUuid: String, ts: Long): NostrEvent {
        ...
        return signer.createSignedEvent(groupId, "expense_delete",
            encryption.encrypt(payload, groupKey), expenseUuid)
    }
    ...
    fun `full real-world simulation - all functionality both phones`() = runBlocking {
        ...
        val meta1Event = p1Signer.createSignedEvent(groupId, "group_meta",
        ...
        val exp1 = makeExpense(
            java.util.UUID.randomUUID().toString(), 150000, "INR",
            "Dinner at restaurant", p1Pub, SplitType.EQUAL,
        ...
        val exp2 = makeExpense(
            java.util.UUID.randomUUID().toString(), 40000, "INR",
            "Cab to airport", p2Pub, SplitType.EXACT,
        ...
        val exp3 = makeExpense(
            java.util.UUID.randomUUID().toString(), 80000, "INR",
            "Groceries from store", p1Pub, SplitType.EQUAL,
        ...
        assertEquals(exp2.id, delPayload["expense_id"]!!.let {
        ...
        val exp2b = makeExpense(
            java.util.UUID.randomUUID().toString(), 35000, "INR",
            "Cab to airport (corrected)", p2Pub, SplitType.EQUAL,
