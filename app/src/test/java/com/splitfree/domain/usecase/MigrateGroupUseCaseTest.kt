...
class MigrateGroupUseCaseTest {
    ...
    private val oldGroup = Group(
        id = oldGroupId, name = "Trip", createdBy = myPubkey,
        createdAt = 1000, members = listOf(myPubkey, memberPubkey, removePubkey),
        relays = listOf("wss://relay.test")
    )

    private val fakeEvent = NostrEvent(
        id = "evt1", pubkey = myPubkey, createdAt = 1000,
        kind = 30078, tags = emptyList(), content = "enc", sig = "sig"
    )
    ...
    @Test
    fun `invoke publishes migrate and meta events`() = runBlocking {
    ...
    @Test
    fun `invoke deletes old group key`() = runBlocking {
        useCase(oldGroupId, removePubkey)
        verify { groupRepo.deleteGroupKey(oldGroupId) }
    }

    @Test
    fun `invoke saves new group`() = runBlocking {
        useCase(oldGroupId, removePubkey)
        coVerify { groupRepo.save(match { it.id != oldGroupId }, "newKey") }
    }

    @Test(expected = IllegalStateException::class)
    fun `invoke fails if group not found`() = runBlocking {
    ...
    @Test(expected = IllegalStateException::class)
    fun `invoke fails if caller is not creator`() = runBlocking {
    ...
    @Test(expected = IllegalStateException::class)
    fun `invoke fails if member not in group`() = runBlocking {
        useCase(oldGroupId, "dd".repeat(32))
        Unit
    }

    @Test(expected = IllegalStateException::class)
    fun `invoke fails if trying to remove self`() = runBlocking {
    ...
    @Test
    fun `handleMigration joins new group`() = runBlocking {
    ...
    @Test
    fun `handleMigration ignores if I was removed`() = runBlocking {
        every { identity.getPublicKeyHex() } returns removePubkey
        val migration = GroupMigration(
            newGroupId = "new-id", encryptedKeys = emptyMap(),
            members = listOf(myPubkey, memberPubkey), removedMember = removePubkey
        )
        useCase.handleMigration(Json.encodeToString(GroupMigration.serializer(), migration), myPubkey, oldGroupId)
    ...
    @Test
    fun `handleMigration ignores if new group already exists`() = runBlocking {
        every { identity.getPublicKeyHex() } returns memberPubkey
        coEvery { groupRepo.getById("new-id") } returns Group("new-id", "G", "", myPubkey, 1000, listOf(myPubkey), listOf("wss://r"))
        val migration = GroupMigration(
            newGroupId = "new-id", encryptedKeys = mapOf(memberPubkey to "k"),
            members = listOf(myPubkey, memberPubkey), removedMember = removePubkey
        )
        useCase.handleMigration(Json.encodeToString(GroupMigration.serializer(), migration), myPubkey, oldGroupId)
        coVerify(exactly = 0) { groupRepo.save(any(), any()) }
    ...
    @Test
    fun `handleMigration rejects from non-creator`() = runBlocking {
        every { identity.getPublicKeyHex() } returns memberPubkey
    ...
    @Test
    fun `handleMigration rejects invalid JSON`() = runBlocking {
        every { identity.getPublicKeyHex() } returns memberPubkey
    ...
    @Test
    fun `handleMigration rejects invalid newGroupId format`() = runBlocking {
        every { identity.getPublicKeyHex() } returns memberPubkey
        coEvery { groupRepo.getById("not-a-uuid") } returns null
        coEvery { groupRepo.getById(oldGroupId) } returns oldGroup
        val migration = GroupMigration(
            newGroupId = "not-a-uuid", encryptedKeys = mapOf(memberPubkey to "k"),
            members = listOf(myPubkey, memberPubkey), removedMember = removePubkey
        )
        useCase.handleMigration(Json.encodeToString(GroupMigration.serializer(), migration), myPubkey, oldGroupId)
        coVerify(exactly = 0) { groupRepo.save(any(), any()) }
