...
class ExportImportUseCaseTest {
    ...
    fun `import stores new events and returns count`() = runBlocking {
        ...
        val importUseCase = ImportGroupUseCase(eventDao, groupRepo, encryption, eventValidator)
    ...
    fun `import rejects invalid HMAC hex`() = runBlocking {
        ...
        ImportGroupUseCase(eventDao, groupRepo, encryption, eventValidator)(badJson)
        Unit
    }
    ...
    fun `import skips event with invalid original JSON signature`() = runBlocking {
        ...
        val count = ImportGroupUseCase(eventDao, groupRepo, encryption, eventValidator)(buildExportJson(events))
        assertEquals(0, count)
    }
